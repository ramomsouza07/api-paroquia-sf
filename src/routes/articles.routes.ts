...
import {
  artigoIdParamsSchema,
  artigoSchema,
  artigoSlugParamsSchema,
  atualizarArtigoBodySchema,
  criarArtigoBodySchema,
  listaArtigosResponseSchema,
  listarArtigosQuerySchema,
} from '../schemas/artigo.schema.js';
...
export async function articleRoutes(app: FastifyInstance) {
  ...
  server.get(
    ...
    async (request, reply) => {
      ...
      if (!artigo) return reply.code(404).send({ erro: 'Artigo não encontrado' });
      return artigo;
    }
  );
  ...
  server.put(
    ...
    async (request, reply) => {
      ...
      if (!atual) return reply.code(404).send({ erro: 'Artigo não encontrado' });
